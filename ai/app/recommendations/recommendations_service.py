...
class RecommendationsService:
    ...
    def get_recommendations(
        self, user_preferences, courses: List[Course], myCoursesIds: List[int]
    ) -> List[Course]:
        # LLM에 전달할 프롬프트 생성
        prompt = build_prompt_for_recommendations(
            user_preferences, courses, myCoursesIds
        )
        try:
            # LLM 호출
    ...
    def _get_courses_by_ids(
        self, course_ids: List[int], all_courses: List[Course]
    ) -> List[Course]:
